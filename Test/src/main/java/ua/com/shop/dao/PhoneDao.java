...
public interface PhoneDao extends JpaRepository<Phone, Integer> {
	...
	@Query("SELECT p FROM Phone p WHERE p.maker.id = ?1 and p.model = ?2 and p.price = ?3 and p.display.id = ?4 and p.primaryCamera = ?5 and p.secondaryCamera = ?6 and p.processor.id = ?7 and p.ram.id = ?8 and p.internal.id = ?9 and p.card.id = ?10 and p.numberOfSimCards.id = ?11 and p.operatingSystem.id = ?12 and p.battery = ?13 and p.color.id = ?14")
	Phone findUnique(int makerId, String model, int price, int displayId,
			...
			int internalId, int cardId, int numberOfSimCardsId,
