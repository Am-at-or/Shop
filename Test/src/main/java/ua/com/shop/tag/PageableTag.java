...
public class PageableTag extends SimpleTagSupport {
	...
	public void doTag() throws JspException, IOException {
		...
		if (last <= VISIBLE) {
			...
			int start = (current - VISIBLE / 2) >= FIRST ? (current - VISIBLE / 2)
					: FIRST;
			int finish = (current + VISIBLE / 2) <= last ? (current + VISIBLE / 2)
					: last;
	...
	private void addAllParameters() {
		...
		for (Entry<String, String[]> entry : map.entrySet()) {
			for (String value : entry.getValue()) {
				if (!(entry.getKey().equals("page") || entry.getKey().equals(
						"size"))) {
					sw.append(AMPER);
	...
	public void setPage(Page<?> page) {
		if (page == null)
			throw new IllegalArgumentException(
					"В контроллері переіменуйте всі атрибути з значенням findAll() на page");
